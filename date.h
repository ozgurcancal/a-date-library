...
public:
	...
	explicit Date(const char* p);  //7		Date nesnesini formatlanmış yazıdan alacağı tarih değeri ile oluşturacak. Format: gg/aa/yil
	explicit Date(std::time_t timer); //8	Date nesnesini "calender time" değerinden dönüştüreceği tarih değeri ile oluşturacak.
